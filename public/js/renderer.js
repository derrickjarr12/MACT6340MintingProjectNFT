...
// --- AI Music Generation Integration (5 Channels) ---
window.addEventListener('DOMContentLoaded', () => {
  const aiGenContainer = document.getElementById('aiGenSlots');
  const aiUrls = new Array(NUM_CHANNELS).fill(null);
  
  // Create 5 AI generation slots
  for (let i = 0; i < NUM_CHANNELS; i++) {
    const slot = document.createElement('div');
    slot.className = 'ai-slot';
    slot.innerHTML = `
      <h4 style="margin:0 0 8px 0;font-size:13px;color:#9db3ff;">Channel ${i+1}</h4>
      <input type="text" class="ai-prompt" placeholder="e.g. lofi beat" data-id="${i}" style="width:100%;padding:6px;background:#0b0b0f;border:1px solid #3a3a56;color:#eaeaf0;border-radius:4px;margin-bottom:8px;font-size:12px;">
      <button class="gen-btn" data-id="${i}" style="width:100%;background:#6a8dff;color:#fff;border:none;padding:8px;border-radius:6px;cursor:pointer;font-size:12px;margin-bottom:6px;transition:all 0.3s ease;">🎵 Generate</button>
      <div class="ai-status" data-id="${i}" style="font-size:11px;margin-bottom:6px;min-height:16px;"></div>
      <button class="load-btn" data-id="${i}" disabled style="width:100%;background:#2b2b40;color:#fff;border:1px solid #3a3a56;padding:8px;border-radius:6px;cursor:pointer;font-size:12px;transition:all 0.3s ease;opacity:0.5;">↓ Load to Sampler</button>
    `;
    slot.style.background = '#1b1b2a';
    slot.style.border = '1px solid #2d2d45';
    slot.style.padding = '12px';
    slot.style.borderRadius = '8px';
    aiGenContainer.appendChild(slot);

    // Generate button listener
    const genBtn = slot.querySelector('.gen-btn');
    genBtn.addEventListener('click', async () => {
      const prompt = slot.querySelector('.ai-prompt').value.trim();
      const status = slot.querySelector('.ai-status');
      ...
      try {
        ...
        if (data.success && data.audioUrl) {
          aiUrls[i] = data.audioUrl;
          status.textContent = '✓ Ready!';
          status.style.color = '#4caf50';
          loadBtn.disabled = false;
          loadBtn.style.background = '#6a8dff';
          loadBtn.style.border = '1px solid #9db3ff';
    ...
    loadBtn.addEventListener('click', async () => {
      ...
      if (!url) {
        status.textContent = '❌ No audio URL';
        status.style.color = '#f44336';
        return;
      }
      ...
      try {
        ...
        const promptText = slot.querySelector('.ai-prompt').value.substring(0, 15);
        drops[i].textContent = `AI: ${promptText}`;
        drops[i].classList.add('loaded');
...
for (let i = 0; i < NUM_CHANNELS; i++) {
  ...
  // Label above the vinyl
  const label = document.createElement('div');
  label.className = 'drop-label';
  label.textContent = `Drop Sample ${i+1}`;
