...
(async () => {
  ...
  app.post("/api/generate-music", async (req, res) => {
    const { prompt, lyrics, model } = req.body;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Mureka API error: ${response.status} ${await response.text()}`);
      }
      const data = await response.json();
      // Expecting data.audio_url or similar in response
      if (data.audio_url) {
        return res.json({
          success: true,
          audioUrl: data.audio_url,
          prompt,
          description: data.description || "Generated by Mureka",
          message: "Generated using Mureka API"
        });
      } else {
        throw new Error("Mureka API did not return audio_url");
      }
    } catch (error) {
      console.error("Mureka AI music generation error:", error);
      ...
      return res.json({
        ...
        prompt,
        description,
        message: "Demo mode: Generated using local audio sample (fallback)",
      });
    }
  });
