...
window.addEventListener('DOMContentLoaded', () => {
  const aiPrompt = document.getElementById('aiMusicPrompt');
  const aiGenBtn = document.getElementById('aiMusicGenerate');
  const aiStatus = document.getElementById('aiMusicStatus');
  const aiResult = document.getElementById('aiMusicResult');
  const aiAudio = document.getElementById('aiMusicAudio');
  const aiDownload = document.getElementById('aiMusicDownload');
  const aiChannelSelect = document.getElementById('aiMusicChannelSelect');
  const aiAddBtn = document.getElementById('aiMusicAddToSampler');
  ...
  for (let i = 0; i < NUM_CHANNELS; i++) {
    ...
    aiChannelSelect.appendChild(opt);
  }
  ...
  aiGenBtn.addEventListener('click', async () => {
    ...
    aiAudioUrl = null;
    try {
      const res = await fetch('http://localhost:3001/api/generate-music', {
        method: 'POST',
      ...
      if (data.success && data.audioUrl) {
        aiAudioUrl = data.audioUrl;
        aiAudio.src = aiAudioUrl;
        aiResult.style.display = '';
        aiStatus.textContent = 'Music generated!';
      } else {
  ...
  aiAddBtn.addEventListener('click', async () => {
    if (!aiAudioUrl) return;
    const channelIdx = parseInt(aiChannelSelect.value);
    aiStatus.textContent = 'Loading into sampler...';
    try {
      const resp = await fetch(aiAudioUrl);
      const arr = await resp.arrayBuffer();
      const audioBuf = await audioCtx.decodeAudioData(arr);
      buffers[channelIdx] = audioBuf;
      // Update drop UI
      const drops = document.querySelectorAll('.drop');
      drops[channelIdx].textContent = 'AI Music';
      drops[channelIdx].classList.add('loaded');
      aiStatus.textContent = 'Loaded into sampler!';
    } catch (err) {
      aiStatus.textContent = 'Failed to load into sampler.';
    }
  });
});
// Audio setup
